...
let
  ...
  kanso-cli = pkgs.stdenv.mkDerivation {
    ...
    installPhase = ''
        ...
        echo "export PYTHONPATH=$out/share/kanso:\$PYTHONPATH" >> $out/bin/kanso-panel
        echo "exec ${python3Env}/bin/python3 $out/share/kanso/core/panel.py \"\$@\"" >> $out/bin/kanso-panel
...
in
{
  ...
  environment.etc."os-release".text = lib.mkForce ''
    ...
    SUPPORT_URL=""
    BUG_REPORT_URL=""
    LOGO="${/kanso/assets/kanso-logo.png}"
  '';

  environment.etc."fastfetch/config.jsonc".text = ''
  {
      "logo": {
          "source": "${/kanso/assets/kanso-logo.png}",
          "type": "kitty"
      },
